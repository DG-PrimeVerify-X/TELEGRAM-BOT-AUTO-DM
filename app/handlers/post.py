...
async def start_post(call: CallbackQuery, state: FSMContext, db, config):
    ...
    chans = await assigned_channels(db, call.from_user.id, config.channel_id)
...
async def choose_channel(call, state, db):
    if not await allowed(db, call.from_user.id, 'posts'): await call.answer('⛔ Access denied', show_alert=True); return
    cid=int(call.data.rsplit(':',1)[1]); await state.update_data(target_type='channel',target_chat_id=cid); await state.set_state(PostStates.waiting_content); await call.message.answer('📣 Send the content now. Text, photo, video, audio, document, ZIP, APK etc.\n\n/cancel to stop.'); await call.answer()
